...
export default function DocumentDetails() {
  ...
  useEffect(() => {
    const fetchDocumentDetails = async () => {
      try {
        ...
        setError(err.response?.data?.detail || 'Failed to pull document parameter matrix from server.');
      } finally {
        setLoading(false);
      }
    };
  ...
  // 3. TEXT UTILITY CLIPBOARD HANDLER
  ...
  // Dynamic Status Badge Color Resolver
  ...
  const isPdf = fileUrl.toLowerCase().includes('.pdf') || document?.filename?.toLowerCase().endsWith('.pdf');

  // ROLE CHECK: If this field is missing from the JSON payload, we treat them as a normal user role
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 text-slate-900 font-sans antialiased">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {/* BACK NAVIGATION AND TITLE ACTION ROW */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            <button 
              onClick={() => navigate(-1)}
              className="text-xs font-bold text-gray-400 hover:text-slate-800 tracking-wider uppercase flex items-center space-x-1 mb-2 transition cursor-pointer"
            >
              <span>← Back</span>
            </button>
            <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 truncate max-w-xl" title={document?.filename}>
              {document?.filename}
            </h1>
          </div>
        </div>

        {/* 🔥 TRACKER PIPELINE FLOW RENDERING STEP */}
        <StatusTracker 
        ...
        <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 mb-8 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm font-semibold">
          <div className="border-r border-slate-100 last:border-0 pr-2">
            <span className="text-xs text-gray-400 block font-medium">Document Model Class</span>
            <span className="text-slate-800 uppercase font-mono text-xs block mt-1">{document?.document_type}</span>
          </div>
          ...
          <div>
            ...
            <span className="text-slate-600 block text-xs mt-1 truncate">{document?.uploaded_at}</span>
          </div>
        </div>

        {/* CORE WORKSPACE WINDOW PANEL GRID */}
        <div className={`grid grid-cols-1 gap-8 items-start ${showAdminOcrPanel ? 'lg:grid-cols-2' : 'max-w-3xl mx-auto'}`}>
          
          {/* 🔥 LEFT COLUMN: REMOVED IFRAME/IMAGE EMBED AND CONVERTED TO AN ACTION BUTTON CARD */}
          <div className="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden flex flex-col min-h-[320px] justify-between">
            <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-sm font-bold text-slate-900">Original File Resource</h3>
            </div>
            ...
            <div className="p-8 flex flex-col items-center justify-center text-center space-y-4 flex-1">
              ...
              <div>
                <h4 className="text-base font-bold text-slate-800 truncate max-w-xs mx-auto">
                  {document?.filename}
                </h4>
                <p className="text-xs text-gray-400 mt-1 uppercase font-mono tracking-wider">
                  Format Type: {isPdf ? 'Portable Document Format (PDF)' : 'Image File'}
                </p>
              </div>
              ...
              <div className="pt-2 w-full max-w-xs">
                <a 
                  ...
                  target="_blank" 
                  rel="noreferrer" 
                  className="w-full inline-flex items-center justify-center px-6 py-3 bg-violet-600 hover:bg-violet-700 active:translate-y-0.5 text-white font-bold text-sm rounded-xl shadow-md shadow-violet-200/50 transition duration-150 text-center cursor-pointer"
                >
            ...
            <div className="bg-slate-50 px-5 py-3 border-t border-slate-100 text-[11px] text-gray-400 font-medium text-center">
              Files open in an isolated sandboxed tab module window.
            </div>
          </div>

          {/* RIGHT COLUMN: RAW OCR CHARACTER STREAM DATA OUTPUT BOX (Admins Only!) */}
          {showAdminOcrPanel && (
            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden flex flex-col min-h-[320px] lg:min-h-[320px] max-h-[500px] animate-fade-in">
              ...
              {/* PLAIN CODE TEXT DATA OUTPUT SPACE */}
              <div className="p-6 flex-1 bg-slate-950 font-mono text-xs leading-relaxed text-emerald-400 select-text overflow-y-auto shadow-inner relative">
                {document?.extracted_text ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-gray-500 font-sans p-6">
                    ...
                    <p className="text-xs max-w-xs mt-1">The system engine couldn't extract legible plaintext metrics from this file specimen layer during initialization execution blocks.</p>
                  </div>
                )}
              </div>
            </div>
