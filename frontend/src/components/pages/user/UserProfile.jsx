      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-1">Manage your identity data sheet, upload authorization assets, and configure email credentials.</p>
          </div>
          <button onClick={() => navigate(-1)} className="text-sm font-semibold text-gray-500 hover:text-slate-800 flex items-center space-x-1 transition cursor-pointer"><span>← Back</span></button>
        </div>

        {actionMessage.text && (
          <div className={`mb-6 p-4 rounded-xl border text-sm font-bold text-center animate-fade-in ${
            actionMessage.isError ? 'bg-rose-50 border-rose-200 text-rose-700' : 'bg-emerald-50 border-emerald-200 text-emerald-700'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-2 flex flex-col space-y-1">
              <button onClick={() => { setActiveTab('INFO'); setActionMessage({ text: '', isError: false }); setIsEditing(false); setIsOtpStep(false); }} className={`w-full text-left px-4 py-2.5 rounded-xl text-xs font-bold transition flex items-center space-x-2 cursor-pointer ${activeTab === 'INFO' ? 'bg-violet-50 text-violet-700' : 'text-slate-600 hover:bg-slate-50'}`}><span>👤</span> <span>User Details</span></button>
              <button onClick={() => { setActiveTab('SECURITY'); setActionMessage({ text: '', isError: false }); setIsEditing(false); setIsOtpStep(false); }} className={`w-full text-left px-4 py-2.5 rounded-xl text-xs font-bold transition flex items-center space-x-2 cursor-pointer ${activeTab === 'SECURITY' ? 'bg-violet-50 text-violet-700' : 'text-slate-600 hover:bg-slate-50'}`}><span>🔑</span> <span>Change Password</span></button>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden min-h-[350px]">
            ...
            {activeTab === 'INFO' && (
              <>
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                  <h3 className="text-sm font-bold text-slate-900">{isEditing ? (isOtpStep ? 'Confirm Verification Code' : 'Modify Your Details') : 'User Details'}</h3>
                  {!isEditing && (
                    <button type="button" onClick={() => setIsEditing(true)} className="px-3.5 py-1.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs rounded-xl transition shadow-sm cursor-pointer">✏️ Edit Profile</button>
                  )}
                </div>
                ...
                {!isOtpStep ? (
                    ...
                    <div className="p-6 space-y-6">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-center pb-4 border-b border-slate-100 last:border-0 last:pb-0">
                        ...
                        <div className="sm:col-span-2">
                          {isEditing ? <input type="text" id="fullname" name="fullname" value={formData.fullname} onChange={handleInputChange} required disabled={isSaving} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500 transition text-slate-800 shadow-inner" /> : <span className="block text-sm font-semibold text-slate-800 bg-slate-50 border border-slate-100 px-4 py-2.5 rounded-xl break-all shadow-inner">{user?.fullname || 'N/A'}</span>}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-center pb-4 border-b border-slate-100 last:border-0 last:pb-0">
                        ...
                        <div className="sm:col-span-2">
                          {isEditing ? <input type="email" id="email" name="email" value={formData.email} onChange={handleInputChange} required disabled={isSaving} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-violet-500 transition text-slate-800 shadow-inner" /> : <span className="block text-sm font-semibold text-slate-800 bg-slate-50 border border-slate-100 px-4 py-2.5 rounded-xl break-all shadow-inner">{user?.email || 'unlinked@identity.vault'}</span>}
                        </div>
                      </div>
                  ...
                  <form onSubmit={handleVerifyOtp}>
                    <div className="p-6 space-y-4 text-center max-w-sm mx-auto">
                      ...
                      <h4 className="text-base font-bold text-slate-800">Email Verification Pending</h4>
                      <p className="text-xs text-gray-400 leading-relaxed">Please check the inbox of your new address: <span className="font-bold text-slate-700">{formData.email}</span> and input the 6-digit confirmation token.</p>
                      ...
                      <input 
                        ...
                        required
                        disabled={isSaving}
                        className="w-full text-center tracking-[0.5em] text-lg font-mono font-bold bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-violet-500 transition shadow-inner" 
                      />
                    </div>

                    <div className="bg-slate-50 px-6 py-4 border-t border-slate-100 flex items-center justify-end space-x-3">
                      <button type="button" disabled={isSaving} onClick={() => { setIsOtpStep(false); setOtpCode(''); setActionMessage({ text: '', isError: false }); }} className="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 font-bold rounded-xl transition cursor-pointer text-xs">Back</button>
                      <button type="submit" disabled={isSaving || otpCode.length < 6} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl transition shadow-md flex items-center justify-center space-x-1.5 cursor-pointer text-xs">
            ...
            {activeTab === 'SECURITY' && (
              <>
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50"><h3 className="text-sm font-bold text-slate-900">Change Password</h3></div>
                  ...
                  <div className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-center">
                      ...
                      <div className="sm:col-span-2 relative">
                        ...
                        <button type="button" onClick={() => setShowPasswords(prev => ({ ...prev, confirm: !prev.confirm }))} className="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-slate-600 focus:outline-none transition cursor-pointer">{showPasswords.confirm ? (<i className="fa-regular fa-eye"></i>) : (<i className="fa-regular fa-eye-slash"></i>)}</button>
                      </div>
                    </div>
                  </div>

                  <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-center">
                    <button type="submit" disabled={isSaving} className="px-5 py-2 bg-violet-600 hover:bg-violet-700 disabled:bg-violet-400 text-white font-bold text-xs rounded-xl transition shadow-md flex items-center justify-center space-x-2 cursor-pointer">
